...
test.describe.parallel('Login tests', () => {
    ...
    test('@E2E - added product to cart, remove product from product list, assertValue and back to HomePage', async ({page}) => {
        loginPage = new LoginPage(page)
        productListPage = new ProductListPage(page)
        cartPage = new CartPage(page)
        checkoutPage = new CheckoutPage(page)
    ...
    test.only('@E2E - added product to cart, remove product from product list, remove products from Cart, assertValue and back to HomePage', async ({page}) => {
        ...
        const numberOfProductToSell = await productListPage.countProducts()
        const addProductsToCart = await productListPage.addProductToCart(6, numberOfProductToSell)
        const removeProductsFromCart = await productListPage.removeProductFromCart(1, addProductsToCart)
        const assertProductListVariable = await productListPage.assertAddedProductsToCart(numberOfProductToSell, addProductsToCart, removeProductsFromCart)
        await productListPage.goToCart()
        await cartPage.cartPageAssert()
        const numberOfProductToRemoveFromCart = await cartPage.removeProductFromCart(2, assertProductListVariable.numberOfProductsInCart)
        const numberOfProductsInCartInt = await cartPage.countProductsInCart()
        await cartPage.assertNumberOfProductsInCart(numberOfProductsInCartInt, assertProductListVariable.numberOfProductsInCart - numberOfProductToRemoveFromCart)
        const sumOfPricesInCartInt = await cartPage.SumOfPricesInCartV2(numberOfProductsInCartInt)
        await cartPage.goToCheckoutPage()
        await checkoutPage.checkoutPageAssertTitle()
        await checkoutPage.fillCheckoutForm(checkoutDataObj1)
        await checkoutPage.goToCheckoutOverviewPage()
