...
export class ProductListPage {
  ...
  constructor(page: Page) {
    this.page = page
    this.productContainerDiv = page.locator(
      "div[id='inventory_container'][data-test='inventory-container']",
    )
    this.productPriceDiv = page.locator(
      "div[data-test='inventory-container']>>div[data-test='inventory-item-price']",
    )
    this.productNameDiv = page.locator(
      "div[data-test='inventory-container']>>div[data-test='inventory-item-name']",
    )
    this.productSortSelect = page.locator(
      "select[data-test='product-sort-container']",
    )
    this.addToCartButton = page.locator(
      "div[data-test='inventory-container']>>div[class='pricebar']>>button",
    )
    this.shoppingCartIcon = page.locator("div[id='shopping_cart_container']")
    this.shoppingCartBadge = page.locator(
      "div[id='shopping_cart_container']>>span[data-test='shopping-cart-badge']",
    )
    this.productTitleSpan = page.locator("span[data-test='title']")
    this.onesieImg = page.locator(
  ...
  async addProductToCart(
    numberOfProductsToAddToCart: number,
    numberOfProductsToSell: number,
  ) {
    await this.productPriceDiv.nth(0).waitFor({ state: 'visible' })
  ...
  async removeProductFromCart(
    numberOfProductsToRemove: number,
    numberOfProductsToAddToCart: number,
  ) {
    await this.productPriceDiv.nth(0).waitFor({ state: 'visible' })
    ...
    if (numberOfProductsToRemove <= numberOfProductsToAddToCart) {
      for (let i = 0; i < numberOfProductsToRemove; i++) {
        const buttontext = await (
          await this.addToCartButton.nth(i).innerText()
        ).trim()
  ...
  async assertAddedProductsToCart(
    numberOfProductsToSell: number,
    numberOfProductsToAddToCart: number,
    numberOfProductsToRemove: number,
  ) {
