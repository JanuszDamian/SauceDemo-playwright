...
export class CheckoutOverviewPage {
    ...
    async assertPrices(totalPriceFromProductList: number) {
        ...
        const subtotalValueText = (await this.subtotalLabel.innerText()).replace("Item total: $", "").trim()
        ...
        const taxValueText = (await this.taxLabel.innerText()).replace("Tax: $", "").trim()
        const taxValueInt = Number(parseFloat(taxValueText))

        const sumSubTotalTaxCalc = subtotalValueInt + taxValueInt
        console.log(`The following values ​​were summed up: ${sumSubTotalTaxCalc} = ${subtotalValueInt} + ${taxValueInt}`)

        const totalPriceText = (await this.totalPriceLabel.innerText()).replace("Total: $", "").trim()
        const totalPriceInt = Number(parseFloat(totalPriceText))
        expect(sumSubTotalTaxCalc).toBeCloseTo(totalPriceInt, 2)
        console.log('The value with tax calculated and downloaded from the website is equal to')
    }
